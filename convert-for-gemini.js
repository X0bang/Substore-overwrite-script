...
function parseCountries(proxies) {
  ...
  for (const proxy of proxies) {
    ...
    if (LANDING_REGEX.test(nodeName)) continue;
...
const dnsConfigBase = {
  ...
  "fallback-filter": {
    ...
    domain: [
      ...
      "+.githubusercontent.com", "+.googlevideo.com",      // === 新增 Google 相关保护 ===
      "+.googleapis.com",
      "+.clients6.google.com",
      "+.doubleclick.net",
      "+.google.dev"    ],
...
function buildRules() {
  const rules = [
    ...
    // === Gemini 强制规则（2026 年最新完整版，已补全所有关键子域）===
    "RULE-SET,OpenAI,OpenAI",
    "RULE-SET,Claude,Claude",
    "RULE-SET,Gemini,Gemini",                    // 先走规则集兜底
    // 针对日志中出现的 oauthaccountmanager + 其他高危 OAuth/API 域，强制走 Gemini（落地）
    "DOMAIN-SUFFIX,oauthaccountmanager.googleapis.com,Gemini",
    "DOMAIN-SUFFIX,oauth2.googleapis.com,Gemini",               // OAuth token 交换
    "DOMAIN-SUFFIX,accounts.google.com,Gemini",
    "DOMAIN-SUFFIX,accounts.youtube.com,Gemini",                // 偶尔交叉
    "DOMAIN-SUFFIX,googleapis.com,Gemini",                      // 广谱覆盖 googleapis 子域（最保险，但可能稍广）
    "DOMAIN-SUFFIX,gemini.google.com,Gemini",
    "DOMAIN-SUFFIX,generativelanguage.googleapis.com,Gemini",
    "DOMAIN-SUFFIX,gstatic.com,Gemini",
    "DOMAIN-SUFFIX,fonts.gstatic.com,Gemini",
    "DOMAIN-SUFFIX,fonts.googleapis.com,Gemini",
    "DOMAIN-SUFFIX,apis.google.com,Gemini",
    "DOMAIN-SUFFIX,www.googleapis.com,Gemini",
    "DOMAIN-SUFFIX,play.google.com,Gemini",
    "DOMAIN-SUFFIX,googleusercontent.com,Gemini",
    "DOMAIN-SUFFIX,lh3.googleusercontent.com,Gemini",
    "DOMAIN-SUFFIX,lh4.googleusercontent.com,Gemini",
    "DOMAIN-SUFFIX,lh5.googleusercontent.com,Gemini",
    "DOMAIN-SUFFIX,ssl.gstatic.com,Gemini",
    "DOMAIN-SUFFIX,clients6.google.com,Gemini",
    "DOMAIN-SUFFIX,oauthaccounts.google.com,Gemini",
    // === 以下为新增关键子域（解决 IP 不一致核心）===
    "DOMAIN-SUFFIX,geller-pa.googleapis.com,Gemini",
    "DOMAIN-SUFFIX,alkalicore-pa.clients6.google.com,Gemini",
    "DOMAIN-SUFFIX,waa-pa.clients6.google.com,Gemini",
    "DOMAIN-SUFFIX,jnn-pa.googleapis.com,Gemini",
    "DOMAIN-SUFFIX,cloudcode-pa.googleapis.com,Gemini",
    "DOMAIN-SUFFIX,proactivebackend-pa.googleapis.com,Gemini",
    "DOMAIN-SUFFIX,aisandbox-pa.googleapis.com,Gemini",
    "DOMAIN-SUFFIX,static.doubleclick.net,Gemini",
    "DOMAIN-SUFFIX,googleads.g.doubleclick.net,Gemini",
    "DOMAIN-SUFFIX,ogs.google.com,Gemini",
    "DOMAIN-SUFFIX,td.doubleclick.net,Gemini",
    "DOMAIN-SUFFIX,ai.google.dev,Gemini",
    "DOMAIN-SUFFIX,antigravity-unleash.goog,Gemini",
    // === 新增结束 ===
...
function buildProxyGroups({ countryGroupNames, countryMap, landingNodes }) {
  ...
  const groups = [
    // 核心选择组
    {
      ...
      type: "select",
      proxies: ["故障转移", ...countryGroupNames, "手动选择", "DIRECT"].filter(Boolean),
    },
    {
      name: "其他流量",
    ...
    // AI 专用
    { name: "OpenAI",     icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/ChatGPT.png",   type: "select", proxies: ["落地节点", ...openaiProxies] },
    { name: "Claude",     icon: "https://gcore.jsdelivr.net/gh/zuluion/Qure/IconSet/Color/Claude.png",            type: "select", proxies: ["落地节点", ...claudeProxies] },
    { name: "Gemini",     icon: "https://gcore.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/AI.png",         type: "select", proxies: ["落地节点", ...geminiProxies] },
    { name: "Perplexity", icon: "https://gcore.jsdelivr.net/gh/zuluion/Qure/IconSet/Color/Perplexity.png",        type: "select", proxies: ["落地节点", ...geminiProxies] },
    { name: "Copilot",    icon: "https://gcore.jsdelivr.net/gh/powerfullz/override-rules@master/icons/Microsoft_Copilot.png", type: "select", proxies: ["落地节点", ...allProxies] },
    ...
    // 科技/工具
    {
      ...
      type: "select",
      proxies: [
        "落地节点",           // 默认优先家宽（防 Gemini 风控）
        "Gemini",             // 直接跟随 Gemini 组的选择
        "美国节点"            // 备选高速方案
      ]
    },
...
function main(params) {
  ...
  const landingNodes      = parseLandingNodes(proxies);
  const countryGroupNames = getCountryGroupNames(countryMap);

  const proxyGroups    = buildProxyGroups({ countryGroupNames, countryMap, landingNodes });
